/*! gradient animation*/
{
    ...
    DOM.register_button = document.querySelector('.register__button');
    ...
    DOM.register_button.addEventListener('click', (ev) =>{if(npos != 5){close(); setTimeout(() => open(5), 400);}});
    let current,npos;
    const open = (pos) => {
        this.isOpen = true;
        anime({
    ...
    const close = () => {
        ...
        const contentInner = DOM.contentInner[npos];
        anime({
            targets: [contentInner.querySelectorAll('.content__title > span'), contentInner.querySelectorAll('.content__subtitle > span'), DOM.ctrlBack],
            delay: (t, i) => anime.random(0, 300),
            duration: 200,
